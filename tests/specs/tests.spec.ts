...
test.describe("Auto-refresh behavior", () => {
    ...
    for (const tab of [
        ...
        test(`Side panel stays open during auto-refresh (${tab.name} tab)`, async ({page, server}) => {
            const client = server.createClient();
            ...
            await page.goto(exe.url);
            await page.getByTestId(tab.tabTestId).click();
            await expect(page.getByTestId(TestId.ExecutionDetailTitle)).toContainText(`side-panel-refresh-${tab.name}`);
