...
impl ArtifactUploader2d {
  pub async fn upload(&self, metadata: &UserMetadataBuilder, data: impl Type2d) -> Result<PublicArtifactId, ClientError> {
    self.base.upload_raw(metadata, data.convert_2d_to_raw()).await
  }
