...
#[test_log::test(tokio::test)]
async fn test_execution_layer_filter_allows_execution() -> anyhow::Result<()> {
  let server = TestServer::new().await;
  let client = server.create_client()?;
  ...
  let app = Router::new()
    .route(
      "/allowed",
      get(|| async {
        observe!("allowed_observation", "Allowed request");
        "Allowed"
      }),
    )
    .layer(
      ExecutionLayer::new(client.clone()).with_filter(|req| {
        // Only allow /allowed path
        req.uri().path() == "/allowed"
      }),
    );
  ...
  let response = reqwest::get(format!("http://{}/allowed", addr)).await?;
  assert_eq!(response.status(), 200);
  assert_eq!(response.text().await?, "Allowed");
  ...
  let observations = server
    ...
    .await?;
  assert_eq!(observations.len(), 1);
  assert_eq!(observations[0].name, "allowed_observation");
